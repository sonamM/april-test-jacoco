...
//		};
	...
	private static void resolveExcelFiles(Class<?> callingTestClass)
		...
		for (String excelFileInfo:excelFilesArg.excelDataFiles()) {
			...
			String excelFileKey=excel[0];
			if(!TestDataContainer.getInstance().hasDataFile(excelFileKey)){
				String excelFilePath=TestBedManagerConfiguration.getInstance().getXlsDataConfig().get(excel[1]);
						
				synchronized (callingTestClass) {
					if(!TestDataContainer.getInstance().hasDataFile(excel[0])){
						TestDataContainer.getInstance().addDataFile(excelFileKey, excelFilePath);
					}
				}
			}
		}
